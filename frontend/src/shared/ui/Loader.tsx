...
const LOADERS = { ring: Ring, spin: Spin };

export interface LoaderProps extends Omit<React.SVGProps<SVGSVGElement>, "display" | "opacity"> {
  variant?: keyof typeof LOADERS;
}

export function Loader({ variant = "spin", className, ...props }: LoaderProps) {
  const LoaderPrimitive = LOADERS[variant];
  return (
    <LoaderPrimitive
      role="presentation"
      aria-label="Loading..."
      className={twMerge(
        ...
        variant === "spin" && "animate-spin stroke-current [animation-duration:0.83333s] [animation-timing-function:linear]",
        className,
      )}
      {...props}
    />
