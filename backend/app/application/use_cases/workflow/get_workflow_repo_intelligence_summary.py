...
from app.infrastructure.services.workflow.workflow_repo_intelligence_projection import (
    build_workflow_repo_hotspots,
    summarize_workflow_repo_hotspots,
)


class GetWorkflowRepoIntelligenceSummaryUseCase:
    ...
    async def execute(self, limit: int = 25) -> dict:
        ...
        hotspots = [item for session in sessions for item in build_workflow_repo_hotspots(session)]
        return summarize_workflow_repo_hotspots(sessions, hotspots)
